...
void printDPsolverResult(KpSolverHeurDP &kp, int sizeMax, int sizeNeighbor, const string& sep, ofstream &outfile) {
...
void printGreedySolverResult(KpSolverGreedy &kp, int sizeMax, const string& sep, ofstream &outfile) {
    if (sizeMax < kp.getNbItems()) {
        if (sizeMax > 0) kp.extractSubInstance(0, sizeMax);
        auto start = std::chrono::steady_clock::now();
        kp.solve();
...
int main() {
    ...
    if (outfile) {
        // instructions
        for (int &itNbItems : sizePb) {
            for (auto &file : files) {
                instanceFile = file.c_str();
                kpGreedy.importInstance(instanceFile);
                if (itNbItems < kpGreedy.getNbItems()) {
                    outfile << file << sep << itNbItems << sep << sep;
