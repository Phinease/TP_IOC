...
void KpSolverBB::printStatus() {
    ...
    gap = (floor(upperBoundOPT) - (double) costSolution) / ((double) costSolution) * 100;
    cout << " gap : " << gap << "%" << endl;
/*
	cout  << "nodes UB: ";
	for (std::deque<NodeBB*>::iterator it = nodes.begin() ; it != nodes.end(); ++it)
...
void KpSolverBB::solve() {
    ...
    while ((nbMaxNodeBB < 0 || nbNodes < nbMaxNodeBB) && !(nodes.empty())) {
        ...
        if (withPrimalHeuristics && nod->getFractionalVariable() > 0.0000001 &&
            nod->getNodeUpperBound() >= costSolution + 1) {
        ...
        if (!(nod->canBeRemoved()) && nod->getFractionalVariable() > 0.0000001 &&
            nod->getNodeUpperBound() >= costSolution + 1) {
            //branchements
        ...
        if (!(nod->canBeRemoved()) && nod->getFractionalVariable() < 0.0000001 &&
            nod->getNodeUpperBound() > costSolution + 0.99999) {
            nod->primalHeuristic(knapsackBound, nbItems, weights, values);
            costSolution = nod->getNodePrimalBound();
            nod->copyPrimalSolution(&solution);
        }
